...
import click.dailyfeed.code.domain.content.post.exception.PostException;
...
@Slf4j
@RestControllerAdvice(basePackages = "click.dailyfeed.content.domain.post.api")
public class PostControllerAdvice {
    @ExceptionHandler(PostException.class)
    public DailyfeedErrorResponse handlePostException(
            PostException e,
            HttpServletRequest request) {

        log.warn("Comment exception occurred: {}, path: {}",
                e.getPostExceptionCode().getReason(),
                request.getRequestURI());

        return DailyfeedErrorResponse.of(
                e.getPostExceptionCode().getCode(),
                ResponseSuccessCode.FAIL,
                e.getPostExceptionCode().getMessage(),
                request.getRequestURI()
        );
    }
